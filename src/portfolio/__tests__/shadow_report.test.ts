...
test("shadow report extracts recent mismatch details", () => {
  const mismatches = extractShadowMismatchDetails([
    {
      actualRoute: "hold",
      shadowRoute: "hold",
      actualDqContracts: 0,
  ...
  assert.equal(mismatches[0]?.actualRoute, "close_long");
  assert.equal(mismatches[1]?.actualRoute, "grid_hold");
});
